...
namespace Yunt.XmlCheck.Models
{
    ...
    public partial class DataType : Entity<DataType>
    {
        ...
        public static IList<DataType> FindAllById(int id)
        {
            if (Meta.Count >= 1000)
                return FindAll(__.Id, id);
            else // 实体缓存
                return Meta.Cache.Entities.FindAll(__.Id, id);
        }

        /// <summary>根据Bit和Desc查找</summary>
        /// <param name="bit"></param>
        /// <param name="desc"></param>
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static DataType FindByBitAndDesc(int bit,string desc)
        {
            if (Meta.Count >= 1000)
                return Find(new string[] { __.Bit, __.Description },
                    new object[] { bit, desc });
            else // 实体缓存
                return Meta.Cache.Entities.Find(e => e.Bit == bit && e.Description == desc);
        }
